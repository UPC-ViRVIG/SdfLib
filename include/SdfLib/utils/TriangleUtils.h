...
namespace TriangleUtils
{
    ...
    inline float getNoSignDistPointAndTriangle(glm::vec3 point, const TriangleData& data, 
                                               glm::vec3 v1, glm::vec3 v2, glm::vec3 v3, glm::vec3& outNormal)
    {
        glm::vec3 projPoint = data.transform * (point - data.origin);
        ...
        if(de1 >= 0)
        {
            if(projPoint.x <= 0) // Its near v1
            {
                outNormal = normalizeVector(point - v1);
                return glm::sqrt(glm::dot(projPoint, projPoint));
            }
            else if(projPoint.x >= data.v2) // Its near v2
            {
                const glm::vec3 p = projPoint - glm::vec3(data.v2, 0.0, 0.0);
                outNormal = normalizeVector(point - v2);
                return glm::sqrt(glm::dot(p, p));
            }
            else // Its near edge 1
            {
                outNormal = normalizeVector(glm::transpose(data.transform) * glm::vec3(0.0, projPoint.y, projPoint.z));
                return glm::sqrt(de1 * de1 + projPoint.z * projPoint.z);
            }
        }
        else if(de2 >= 0)
        {
            ...
            {
                const glm::vec3 p = projPoint - glm::vec3(data.v3.x, data.v3.y, 0.0);
                outNormal = normalizeVector(point - v3);
                return glm::sqrt(glm::dot(p, p));
            }
            else // Its near edge 2
            {
                const float dot = (projPoint.x - data.v2) * data.b.x + projPoint.y * data.b.y;
                outNormal = normalizeVector(glm::transpose(data.transform) * glm::vec3((projPoint.x - data.v2) - dot * data.b.x,
                                                                                              projPoint.y - dot * data.b.y, 
                                                                                              projPoint.z));
                return glm::sqrt(de2 * de2 + projPoint.z * projPoint.z);
            }
        }
        else if(de3 >= 0)
        {
            ...
            {
				const float dot = projPoint.x * data.c.x + projPoint.y * data.c.y;
                outNormal = normalizeVector(glm::transpose(data.transform) * glm::vec3(projPoint.x - dot * data.c.x,
                                                                                             projPoint.y - dot * data.c.y, 
                                                                                             projPoint.z));
                return glm::sqrt(de3 * de3 + projPoint.z * projPoint.z);
            }
        }

        outNormal = data.getTriangleNormal();
        {
            const glm::vec3 p = point - v1;
            outNormal = glm::sign(glm::dot(outNormal, p)) * outNormal;
        }
        return glm::abs(projPoint.z);
    }
